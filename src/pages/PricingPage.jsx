...
export default function PricingPage() {
  ...
  const isFreeCurrent = subscription?.plan === 'free' || subscription?.status === 'cancelled';
  const isProfessionalActive = subscription?.plan === 'professional' && subscription?.status === 'active';
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
      <div className="container mx-auto px-4 py-12">
          ...
          <div className="mb-8 flex justify-center">
            <div className="bg-blue-100 border border-blue-200 rounded-lg px-4 py-2">
              <p className="text-blue-900 font-semibold">
                Current Plan: <span className="capitalize">{subscription.status === 'cancelled' ? 'free' : subscription.plan}</span>
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto mb-12">
          <Card className={`relative ${isFreeCurrent ? 'ring-2 ring-blue-500' : ''}`}>
            {isFreeCurrent && (
            ...
            <CardHeader className={isFreeCurrent ? 'pt-16' : ''}>
            ...
            <CardContent className="space-y-6">
              ...
              <Button disabled={isFreeCurrent} className="w-full" variant={isFreeCurrent ? 'outline' : 'secondary'}>
          ...
          <Card className={`relative ${isProfessionalActive ? 'ring-2 ring-blue-500' : 'border-blue-200'}`}>
            {isProfessionalActive && (
              <div className="absolute top-0 left-0 right-0 bg-blue-500 text-white py-2 text-center rounded-t-lg">
                <span className="text-sm font-semibold">Current Plan</span>
              </div>
            )}
            <CardHeader className={isProfessionalActive ? 'pt-16' : ''}>
