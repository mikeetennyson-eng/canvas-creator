              ...
              <div className={`rounded-2xl border p-8 shadow-lg ${
                  ...
                  <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="text-2xl font-bold capitalize">
                          {subscription.status === 'cancelled' ? 'Free (Cancelled)' : `${subscription.plan} Plan`}
                        </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {subscription.plan === 'professional' && subscription.status === 'active' ? '₹400' : '₹0'}
                      <span className="text-base font-normal text-muted-foreground"> {(subscription.plan === 'professional' && subscription.status === 'active') ? '/month' : ''}</span>
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground mb-1">Icons per Project</p>
                    <p className="text-2xl font-bold">
                      {(subscription.plan === 'professional' && subscription.status === 'active') ? 'Unlimited' : '20'}
                    </p>
                  </div>
                  {(subscription.plan === 'professional' && subscription.status === 'active') && (
                    <div>
                ...
                {subscription.plan === 'professional' && subscription.status === 'active' && (
                  <div className="bg-background/50 rounded-lg p-4 mb-6">
                    ...
                    <p className="text-sm text-muted-foreground">Your professional access expires on:</p>
                ...
                <div className="flex gap-3">
                  {(subscription.plan === 'free' || subscription.status === 'cancelled' || subscription.status === 'expired') && (
                    <Button
